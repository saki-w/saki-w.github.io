...
$stmt_filtering = $dbh->prepare("SELECT recipes.id AS recipe_id FROM recipes JOIN materials ON recipes.id = materials.recipe_id JOIN users ON recipes.user_id = users.id WHERE recipes.title LIKE :name OR materials.name LIKE :name OR users.name LIKE :name GROUP BY recipes.id");
...
//------------------------------------------------------------材料取得------------------------------------------------------------
foreach($dbh->query('SELECT recipes.id, materials.name AS materials_name, materials.quantity, unit.name AS unit_name FROM recipes JOIN materials ON recipes.id = materials.recipe_id LEFT JOIN unit ON materials.unit_id = unit.id') as $row1) {
$rows1[] = $row1;
}
//------------------------------------------------------------作り方取得------------------------------------------------------------
